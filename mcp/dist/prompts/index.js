...
export function registerPrompts(server) {
    // ─── cross_chain_swap ──────────────────────────────────────────────────────
    server.prompt('cross_chain_swap', 'Generate a step-by-step plan to execute a cross-chain token swap on HopFast', {
...
- Do NOT submit the transaction yourself — I will sign it with my wallet.`,
    ...
    // ─── find_yield ────────────────────────────────────────────────────────────
    server.prompt('find_yield', 'Help a user discover and deposit into the best yield vault for their assets', {
        walletAddress: z.string().describe("User's wallet address."),
...
Do not proceed past step 4 without my explicit approval.`,
    ...
    // ─── portfolio_review ──────────────────────────────────────────────────────
    server.prompt('portfolio_review', "Review a user's complete HopFast DeFi portfolio: swap history and earn positions", {
        walletAddress: z.string().describe("User's wallet address."),
    }, ({ walletAddress }) => ({
        messages: [
            {
...
4. Highlight any interesting patterns or opportunities (e.g. "You've been swapping USDC
   frequently — consider putting some in a USDC vault on Base for yield").`,
    ...
    // ─── check_swap_status ────────────────────────────────────────────────────
    server.prompt('check_swap_status', 'Monitor a pending cross-chain swap until it completes or fails', {
