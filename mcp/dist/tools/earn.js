...
const CHAIN_KEYS = ['ethereum', 'base', 'bsc', 'polygon', 'monad'];
...
export function registerEarnTools(server) {
    // ─── get_earn_vaults ───────────────────────────────────────────────────────
    server.tool('get_earn_vaults', `Browse yield-earning vaults aggregated by HopFast via LI.FI.
...
Sort by "apy" to find highest yields, or "tvl" for most established vaults.`, {
        ...
        if (chainKey)
            query.set('chainId', String(CHAIN_IDS[chainKey]));
        if (asset)
            query.set('asset', asset);
        if (protocol)
            query.set('protocol', protocol);
        if (minTvlUsd !== undefined)
            query.set('minTvlUsd', String(minTvlUsd));
        if (sortBy)
            query.set('sortBy', sortBy);
        if (limit !== undefined)
            query.set('limit', String(limit));
        const result = await hopfastFetch(`/api/earn/vaults?${query}`);
        return {
            content: [
                {
                    type: 'text',
    ...
    // ─── get_earn_quote ────────────────────────────────────────────────────────
    server.tool('get_earn_quote', `Get a quote for depositing tokens into a yield vault.
...
as toTokenAddress and the user's token address as fromTokenAddress.`, {
        ...
        const result = await hopfastFetch('/api/earn/quote', {
            method: 'POST',
    ...
    // ─── get_earn_positions ────────────────────────────────────────────────────
    server.tool('get_earn_positions', `Retrieve a user's live DeFi yield positions with real-time on-chain balances.
...
Shows current balance, USD value, deposited amount, protocol, chain, and tx hash.`, {
        ...
        const result = await hopfastFetch(`/api/earn/portfolio/${walletAddress}`);
        return {
            content: [
                {
                    type: 'text',
    ...
    // ─── record_earn_deposit ───────────────────────────────────────────────────
    server.tool('record_earn_deposit', `Save a completed vault deposit to HopFast so it appears in the user's
...
is confirmed on-chain.`, {
    ...
    }, async ({ walletAddress, vaultAddress, vaultName, chainId, network, protocolName, tokenSymbol, tokenAddress, amount, amountRaw, txHash, }) => {
        const body = {
        ...
        if (protocolName)
            body.protocolName = protocolName;
        if (tokenSymbol)
            body.tokenSymbol = tokenSymbol;
        if (tokenAddress)
            body.tokenAddress = tokenAddress;
        const result = await hopfastFetch('/api/earn/positions', {
            method: 'POST',
            body: JSON.stringify(body),
        });
        return {
    ...
    server.tool('get_user_preferences', `Fetch a user's saved yield preferences from HopFast.
...
Use this before recommending vaults to personalise suggestions.`, {
        ...
        const result = await hopfastFetch(`/api/earn/preferences/${walletAddress}`);
        return {
            content: [
                {
                    type: 'text',
    ...
    server.tool('save_user_preferences', `Save or update a user's yield preferences in HopFast.
...
- experienceLevel: "beginner" | "intermediate" | "advanced"`, {
        ...
        if (preferredAsset)
            body.preferredAsset = preferredAsset;
        const result = await hopfastFetch('/api/earn/preferences', {
            method: 'POST',
            body: JSON.stringify(body),
        });
        return {
