...
const PROVIDERS = ['lifi', 'squid', 'debridge'];
export function registerSwapTools(server) {
    // ─── get_swap_quote ────────────────────────────────────────────────────────
    server.tool('get_swap_quote', `Get a cross-chain swap quote from one of HopFast's three routing providers
...
chain-native assets like ETH, BNB, POL, MON.`, {
        ...
        const result = await hopfastFetch(`/api/quotes?${query}`, { method: 'POST', body: JSON.stringify(body) });
        return {
            content: [
                {
                    type: 'text',
    ...
    server.tool('get_transaction_status', `Poll the status of a submitted cross-chain swap or bridge transaction.
...
The response also includes receivingTxHash (destination chain tx) and explorerLink.`, {
        ...
        const result = await hopfastFetch(`/api/status?${query}`);
        return {
            content: [
                {
                    type: 'text',
    ...
    server.tool('get_transaction_history', `Retrieve a wallet's swap and bridge transaction history stored in HopFast.
Returns the most recent transactions first, including status, chains, tokens, amounts, and tx hashes.`, {
        ...
        const result = await hopfastFetch(`/api/transactions?${query}`);
        return {
            content: [
                {
                    type: 'text',
    ...
    // ─── record_transaction ────────────────────────────────────────────────────
    server.tool('record_transaction', `Save a submitted swap transaction to the HopFast database so it appears in
...
and broadcasts a swap transaction.`, {
    ...
    }, async ({ walletAddress, txHash, fromChain, toChain, fromToken, toToken, amount, provider, volumeUsd, status, }) => {
        const body = {
        ...
        if (volumeUsd !== undefined)
            body.volumeUsd = volumeUsd;
        if (status)
            body.status = status;
        const result = await hopfastFetch('/api/transactions', {
            method: 'POST',
