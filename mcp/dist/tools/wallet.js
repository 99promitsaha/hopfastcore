...
export function registerWalletTools(server) {
    // ─── register_wallet ───────────────────────────────────────────────────────
    server.tool('register_wallet', `Register a wallet address with HopFast. This creates or updates the wallet
...
It's safe to call multiple times — it's idempotent (upsert).`, {
        ...
        const result = await hopfastFetch('/api/wallets', {
            method: 'POST',
    ...
    // ─── get_protocol_stats ────────────────────────────────────────────────────
    server.tool('get_protocol_stats', `Get aggregated HopFast protocol statistics for a given time period.
Returns:
...
Use for reporting, analytics, or when users ask about HopFast's activity.`, {
        ...
        const result = await hopfastFetch(`/api/stats?${query}`);
        return {
            content: [
                {
                    type: 'text',
    ...
    // ─── check_health ──────────────────────────────────────────────────────────
    server.tool('check_health', `Check if the HopFast backend is online and the database is connected.
Returns service status and database connection status.
Use this to verify connectivity before attempting other operations.`, {}, async () => {
        const result = await hopfastFetch('/api/health');
        return {
            content: [
                {
                    type: 'text',
